...
if matches:
    ...
    if st.button(
        ...
        disabled=not search_selected_seqs,
        key="add_search_selection",
    ):
...
applicant_ready = bool(st.session_state.groups["applicant"]["seqs"])
if not applicant_ready and selected_seqs:
    st.warning("조회하려면 신청의약품 그룹에 품목을 하나 이상 추가하세요.")

if st.button("그룹 전체 품목 조회", type="primary", disabled=not selected_seqs or not applicant_ready or not (mfds_key and hira_key)):
...
if st.session_state.last_result is not None:
    ...
    with comparison_tab:
        if len(filtered_result_df) < 2:
            ...
            st.caption("행은 조회 항목, 열은 그룹·함량별 의약품입니다. 헤더 경계를 드래그해 컬럼 너비를 조절할 수 있습니다.")
